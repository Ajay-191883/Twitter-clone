...
function openModal(modalName, selectedPost, pinId = null) {
  ...
  document
    .getElementById(`${modalName}SelectedPost`)
    .insertAdjacentHTML("afterbegin", selectedPost.outerHTML);
  if (!pinId)
    document
      .getElementById(`${modalName}SubmitButton`)
      .setAttribute("data-id", selectedPost.dataset.id);
  else
    document
      .getElementById(`${modalName}SubmitButton`)
      .setAttribute("data-id", pinId);
}
...
function createPost(postData, mainPost) {
  ...
  let retweetText = "";
  if (isRetweet) {
    const retweetTime = timeDifference(
      new Date(),
  ...
  const replyText = isReply
    ? `Replied to <a href="/profile/${postData.replyTo.postedBy?.username}"> @${postData.replyTo.postedBy?.username} </a>`
    : "";
  const time = timeDifference(new Date(), new Date(postData.createdAt));
  let retweetClass = postData.retweetUsers.includes(
    JSON.parse(userLoggedIn)._id
  )
    ? "active-retweet"
    : "";
  const heartClass = postData.likes.includes(JSON.parse(userLoggedIn)._id)
    ? "fas"
    : "far";
  return `
        ...
        <div class="tweet-container">
            <div class="profile-pic-container">
                <img class="profile-pic" src="${
                  postData.postedBy.profilePic
                }" alt="User's profile pic" class="profile-pic">
            </div>
            <div class="tweet">
                <a href="/profile/${postData.postedBy.username}">
                    <span class="name">${postData.postedBy.firstName} ${
    postData.postedBy.lastName
        ...
        <div class="footer">
            ...
            <button class="retweet"><i class="fas fa-retweet ${retweetClass}"></i><span class="retweet-number">${
    postData.retweetUsers.length || ""
  }</span></button>
            <button class="heart"><i class="${heartClass} fa-heart"></i><span class="like-number">${
...
$(document).on("click", ".heart", function (event) {
  ...
  $.ajax({
    ...
    success: function (postData) {
      event.target.querySelector(".like-number").textContent =
        postData.likes.length || "";
...
$(document).on("click", ".retweet", function (event) {
  ...
  $.ajax({
    ...
    type: "POST",
    success: function (postData) {
      event.target.querySelector(".retweet-number").textContent =
        postData.retweetUsers.length || "";
      if (postData.retweetUsers.includes(JSON.parse(userLoggedIn)._id)) {
        event.target
          .querySelector(".fa-retweet")
          .classList.add("active-retweet");
        emitNotification(postData.postedBy);
      } else {
        event.target
          .querySelector(".fa-retweet")
          .classList.remove("active-retweet");
      }
      displayPosts();
    },
  });
...
// Closing reply Modal
document
  .getElementById("replyCloseModal")
...
$(document).on("click", ".pin", function (event) {
  const selectedPost = event.target.closest(".post");
  const pinId = event.target.closest(".post").querySelector(".pin-container")
    .dataset.id;
  console.log(pinId);
...
document
  .getElementById("unpinCloseModal")
  .addEventListener("click", function () {
    closeModal("unpin");
  });
...
document
  .getElementById("deleteCloseModal")
  .addEventListener("click", function () {
    closeModal("delete");
  });
